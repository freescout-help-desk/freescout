...
class OpenController extends Controller
{
    ...
    public function setThreadAsRead($conversation_id, $thread_id, $hash)
    {
        ...
        if ((int)$thread->conversation_id !== (int)$conversation_id) {
            $mark_as_read = false;
        } elseif (!\Helper::hashEquals($hash, Thread::getOpenTrackingHash($thread, $conversation, $conversation->mailbox))) {
            $mark_as_read = false;
        }
    ...
    public function downloadAttachment($dir_1, $dir_2, $dir_3, $file_name, Request $request)
    {
        ...
        // Links to attachments without id/token have been disabled.
        // https://github.com/freescout-help-desk/freescout/security/advisories/GHSA-wg74-ww4w-2qpc
        if (!$id) {
            return \Helper::denyAccess();
        }

        return $this->downloadAttachmentById($id, $request->query('token', ''));
    }

    /**
     * Download an attachment by id/token - the current, canonical attachment URL
     * format, generated by Attachment::url().
     */
    public function downloadAttachmentById($id, $token)
    {
        $attachment = Attachment::findOrFail($id);

        if (!\Helper::hashEquals($token, $attachment->getToken())) {
            return \Helper::denyAccess();
        }
    ...
    private function serveAttachment(Attachment $attachment)
    {

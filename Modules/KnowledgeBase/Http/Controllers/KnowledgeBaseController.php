...
class KnowledgeBaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($user){
          ...
          return view('knowledgebase::frontend/index', [
              'mailboxes' => $mailboxes,
          ]);
        } else {
    ...
    public function frontend(Request $request, $mailbox_id, $kb_locale = '')
    {
        ...
        if (!count($categories)) {
            ...
            // No need to exclude Private articles, as there are no categories.
        }
    ...
    public function frontendSearch(Request $request, $mailbox_id, $kb_locale = '')
    {
        ...
        $articles = \KbArticle::where('mailbox_id', $mailbox->id)
            ...
            ->where(function ($query_like) use ($like) {
                $query_like->where('title', 'like', $like)
        ...
        foreach ($articles as $i => $article) {
            ...
            if (!stristr($article->title, $q) && !stristr($article->text, $q)) {
                unset($articles[$i]);
            }
        }

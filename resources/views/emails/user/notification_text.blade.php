...
@if (count($threads) == 1){{ __('Received a new conversation') }}@else @if ($thread->action_type == App\Thread::ACTION_TYPE_STATUS_CHANGED){{ __(":person marked as :status conversation", ['person' => $thread->getCreatedBy(true)->getFullName(true), 'status' => $thread->getStatusName()]) }}@elseif ($thread->action_type == App\Thread::ACTION_TYPE_USER_CHANGED)
@include('emails/user/thread_by') {{ __("assigned to :person conversation", ['person' => $thread->getAssigneeName(false, $user)]) }}@elseif ($thread->type == App\Thread::TYPE_NOTE){!! __(":person added a note to conversation", ['person' => '<strong>'.$thread->getCreatedBy(true)->getFullName(true).'</strong>']) !!}@else
{{ __(":person replied to conversation", ['person' => $thread->getCreatedBy(true)->getFullName(true)]) }}@endif @endif #{{ $conversation->number }}
...
## {!! __(':person added a note', ['person' => $thread->getCreatedBy(true)->getFullName(true)]) !!}, {{ __('on :date', ['date' => App\Customer::dateFormat($thread->created_at, 'M j @ H:i').' ('.\Config::get('app.timezone').')' ]) }}@else
## @if ($thread->isForwarded()){{ __(':person forwarded a conversation :forward_parent_conversation_number', ['person' => $thread->getCreatedBy(true)->getFullName(true), 'forward_parent_conversation_number' => '#'.$thread->getMetaFw(App\Thread::META_FORWARD_PARENT_CONVERSATION_NUMBER)]) }}@elseif ($loop->last){{ __(':person started the conversation', ['person' => $thread->getCreatedBy(true)->getFullName(true)]) }}@else{{ __(':person replied', ['person' => $thread->getCreatedBy(true)->getFullName(true)]) }}@endif, {{ __('on :date', ['date' => App\Customer::dateFormat($thread->created_at, 'M j @ H:i').' ('.\Config::get('app.timezone').')' ]) }}@endif:

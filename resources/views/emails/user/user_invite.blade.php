...
@section('content')
	<p style="color:#72808e;font:400 20px/26px 'Helvetica Neue',Helvetica,Arial,sans-serif;padding:0">
	  {!! __h('Hi :user, an account has been created for you at :app_url', ['user' => '<strong style="color:#1e1e1e">'.htmlspecialchars($user->getFullName()).'</strong>', 'app_url' => '<a href="'.htmlspecialchars(\Config::get('app.url')).'" style="border:0;color:#3197d6;text-decoration:none" target="_blank">'.htmlspecialchars(parse_url(\Config::get('app.url'), PHP_URL_HOST)).'</a>']) !!}
	</p>
	<p>
